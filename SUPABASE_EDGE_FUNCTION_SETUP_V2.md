# Supabase Edge Function 설정 가이드 (수정된 버전)
...
```typescript
// Supabase Edge Function - YouTube 다운로더 API (수정된 버전 v2)
...
// YouTube URL에서 비디오 ID 추출하는 함수 (수정된 버전)
...
1. **정규표현식 오류 수정**: `extractVideoId` 함수 개선
2. **에러 처리 강화**: try-catch 블록 추가
3. **코드 안정성 향상**: null 체크 및 에러 핸들링 개선
